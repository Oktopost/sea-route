require('../../namespace').namespace('SeaRoute.parsers', function(root) {
	...
	var PathParser = {
		...
		_setParam: function (name, value, config) {
			...
			if (is.string(param) || 
				...
				is.defined(config[name].type)) {
					throw 'Can not define parameter type both in Path and in config. For parameter "' + name + '"';
			} else {
		...
		_regexParam: function (name, regex, config) {
			...
			if (lastIndex <= 1) {
				throw 'Invalid regex expresion: "' + regex + '"';
			}
		...
		_arrayParam: function (name, valuesString, config) {
			if (valuesString[valuesString.length - 1] !== ')') {
				throw 'Invalid array config for parameter named "' + name + '"';
			}
		...
		_predefinedParam: function (name, predefined, config, predefinedCollection) {
			...
			if (is.undefined(predefinedCollection[predefined])) {
				throw 'There is no predefined parameter named "' + predefined + '"';
			}
		...
		_parseParam: function (param, config, preDefinedParams) {
			...
			if (name.length === 0) {
				throw 'Parameter name not provided';
