...
suite('ParamParser', () =>
{
	...
	suite('create by type', () =>
	{
		...
		test('number', () =>
		{
			var obj = ParamParser.parse('a', { type: 'number' });
			assert.instanceOf(obj, NumberParam);
		});
		
		test('number as num', () =>
		{
			var obj = ParamParser.parse('a', { type: 'num' });
			assert.instanceOf(obj, NumberParam);
		});
		...
		test('wildcard as wild', () =>
		{
			var obj = ParamParser.parse('a', { type: 'wild', exp: 'a*b' });
			assert.instanceOf(obj, WildcardParam);
		});
	...
	suite('Number param', () =>
	{
		...
		test('Range set', () =>
		{
			var obj = ParamParser.parse('a', { type: 'number', min: 2.2, max: 10 });
			
			assert.isTrue(obj.validate(2.3));
			assert.isTrue(obj.validate(10));
			assert.isFalse(obj.validate(2.1));
			assert.isFalse(obj.validate(11));
		});
	});
