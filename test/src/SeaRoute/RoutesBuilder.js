...
suite('RoutesBuilder', () => {
	...
	test('(string path, { params: a, callback }) - params defined in config used', () => {
		...
		assert.instanceOf(result.path().parts()[0].getParam(), SeaRoute.ParamType.RegexParam)
	});
	...
	test('predefined parameter used', () => {
		var predefined = { a: new SeaRoute.ParamType.RegexParam('a', /a/) };
		var builder = new RoutesBuilder(predefined);
		
		var result = builder.create('/{a|[a]}', { callback: () => {} });
		var param = result.path().parts()[0].getParam();
		
		assert.instanceOf(param, SeaRoute.ParamType.PredefinedParamDecorator);
	...
	suite('addParams', () => {
		...
		test('Single parameter added ', () => {
			var predefined = new SeaRoute.ParamType.RegexParam('a', /a/);
			var builder = new RoutesBuilder();
		...
		test('Predefined parameter parsed', () => {
			var predefined = { a: /a/ };
			var builder = new RoutesBuilder();
