...
suite('Query', () => 
{
	...
	suite('parseParameters', () => 
	{
		...
		test('Parameter not defined in strict mode, error thrown', () => 
		{
			...
			assert.throws(() => { query.parseParameters({a: 'a'}); });
		});
		...
		test('Missing required parameter, error thrown', () => 
		{
			...
			assert.throws(() => { query.parseParameters({}); });
		});
	...
	suite('parseParameters', () => 
	{
		test('Empty Query, Return empty array', () => 
		{
			var query = new Query();
			
			assert.deepEqual([], query.paramNames());
		});
		
		test('Single parameter Query, Return array with name', () => 
		{
			var query = new Query();
			
			query.add(new Param('hello'));
			
			assert.deepEqual(['hello'], query.paramNames());
		});
		
		test('Multiple parameters query', () => 
		{
			var query = new Query();
			
			query.add([new Param('hello'), new Param('world')]);
			
			assert.deepEqual(['hello', 'world'], query.paramNames());
		});
	});
});